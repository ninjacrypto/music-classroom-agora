...
export default class WhiteboardBrushColor extends Component<WhiteboardBrushColorProps> {
  ...
  get colors() {
    return WhiteboardBrushColor.colors.map((color) => (
      <WhiteboardBrushColorItem color={color} active={this.isActiveColor(color)} handleClick={this.props.handleChange} key={color.name} />
    ));
  }

  render() {
    return <div className={styles.whiteboardBrushColor}>{this.colors}</div>;
  }
}

...
export const WaveArmSender: FC<BaseWaveArmProps> = observer(({ isH5, isOnPodium, onOffPodium }) => {
  const { handUpUIStore, shareUIStore, layoutUIStore } = useStore();
  const {
    waveArm,
    teacherUuid,
    waveArmDurationTime,
    ...
    fsm.whenAfter(WaveArmStateEnum.waveArmBefore, WaveArmStateEnum.waveArming, () => {
      ...
      promise = new Promise(async (resolve) => {
        task = Scheduler.shared.addPollingTask(async () => {
          await waveArm(teacherUuid, 30, { userName });
        }, Scheduler.Duration.second(30));
        resolve();
      });
    });
  ...
  const handleMouseDown = () => {
    ...
    setFirstTip(true);
    fsm.changeState(WaveArmStateEnum.waveArming, 3);
  };
  ...
  const handleMouseUp = () => {
    ...
    fsm.changeState(WaveArmStateEnum.waveArmAfter, 3);
  };

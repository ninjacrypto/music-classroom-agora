...
export const TextInputTransparent: FC<CommonProps> = ({ placeholder, value, onChange = () => { }, readOnly, error, onBlur, onKeyDown, onKeyUp }) => {
    const [focused, setFocused] = useState(false);
    const inputRef = useRef<HTMLInputElement>(null);
    const handleFocus = () => {
        setFocused(true);
    ...
    const cls = classNames("preset-perf-roadie", {
        error: !!error
    });

    return (
        <div className={cls}>
            <input ref={inputRef} readOnly={readOnly} className="w-full preset-perf-roadie" placeholder={placeholder} style={{ padding: '10px 12px 10px 12px' }} onFocus={handleFocus} onBlur={handleBlur} value={value} onChange={handleChange} onKeyDown={onKeyDown} onKeyUp={onKeyUp} />
        </div>
    );
}

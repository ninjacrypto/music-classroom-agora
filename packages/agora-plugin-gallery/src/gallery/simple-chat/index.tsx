...
import { MusicFunRtmContextProvider } from '@/mf-rtm';
...
const App = observer(() => {
  ...
  const roomIndex = globalStore.findIndex((item: any) =>
    item?.roomId ? item?.roomId === EduClassroomConfig.shared.sessionInfo.roomName : null,
  );

  const chatMinimize = roomIndex > -1 ? globalStore[roomIndex]['chatMinimized'] : true;

  const { publishChatWidget } = useMusicFunRtmContext();

  // console.log('this is console from chat >>>>>>>>', chatMinimize);
  return (
    <div
      id="netless-white"
      ...
      {widgetStore.classroomConfig.sessionInfo.roomType === 0 ? (
        ...
        <ChatNew
          className={classnames('small-class-chat', { 'small-class-chat-fixed': !chatMinimize })}
          collapse={chatMinimize}
          onCanChattingChange={onCanChattingChange}
          canChatting={canChatting}
          isHost={isHost}
          ...
          onSend={handleSendText}
          showCloseIcon={
            widgetStore.classroomConfig.sessionInfo.roomType === 2 ? false : !chatMinimize
          }
          //@ts-ignore
